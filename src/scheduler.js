...
export function startScheduler(client) {
  ...
  if (TEST_MODE) {
    ...
    // Check every minute: close expired contests, send reminders/warnings, auto-reopen
    tasks.push(cron.schedule('* * * * *', () => checkContests(client)));
    // Daily promo classement at 18h every day except Wednesday
...
export async function checkContests(client) {
  ...
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      // Reminder N minutes before end (configurable, default 60min)
      const reminderMs = (contestSettings?.reminder_minutes_before_end ?? 60) * 60000;
      if (!contest.reminder_sent && msLeft > 0 && msLeft <= reminderMs) {
        ...
        console.log(`[SCHEDULER] Rappel envoyé`);
      }

      // Warning X minutes before end (configurable, default 5min)
      ...
      if (!contest.warning_sent && msLeft > 0 && msLeft <= warningMs) {
        ...
        console.log(`[SCHEDULER] Warning envoyé`);
      }

      // Tiebreak: check if resolved or expired
